package entity;

import main.GamePanel;
import java.awt.*;

public class Inky extends Entity
{
//    GamePanel gp;

    public Inky(GamePanel gp)
    {
        this.gp = gp;
        x = 0;
        y = 0;
        speed = 0;
        image = null;
        collisionOnUp = false;
        collisionOnDown = false;
        collisionOnLeft = false;
        collisionOnRight = false;
        direction = "";
        lastDirection = "";

        ghostState = "idle"; /* default: idle */
        idleTime = 480 + 360; /* idles for 6 seconds after pinky (14 seconds) */
        frightenedTag = 0;

        setDefaultValues();
        getImages();
        hitbox = new Rectangle(x, y, gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        killHitbox = new Rectangle(x + 8, y + 8, gp.displayedTileSize - 8, gp.displayedTileSize - 8);
        targetX = x;
        targetY = y;
        movementCooldownTimer = 0;
        wallImmunity = false;

        ghosts[2] = this;
    }

    /* sets default values for Inky */
    @Override
    public void setDefaultValues()
    {
        this.x = 180; /* default: 180 */
        this.y = 260; /* default: 260 */
        this.speed = 1;

        this.direction = "down"; /* default: down */
        this.lastDirection = "down"; /* default: down */
    }

    /* sets up the images for Inky */
    @Override
    public void getImages()
    {
        up1 = setupImage("InkyUp1", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        up2 = setupImage("InkyUp2", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        down1 = setupImage("InkyDown1", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        down2 = setupImage("InkyDown2", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        right1 = setupImage("InkyRight1", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        right2 = setupImage("InkyRight2", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        left1 = setupImage("InkyLeft1", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
        left2 = setupImage("InkyLeft2", "/ghosts/inky/", gp.displayedTileSize + 8, gp.displayedTileSize + 8);
    }

    /* calculates ghost's target position */
    @Override
    public void calculateTarget()
    {
        if (this.ghostState == "chase")
        {
            /* finds an intermediate tile using pinky's chase target algorithm (32 pixels instead of 64),
            then calculates the x and y distance between said tile and Blinky, and finally subtracts it
            subtracts those values from the intermediate tile's values to find the target */
            int intermediateValueX = 0;
            int intermediateValueY = 0;
            int blinkyDistanceX = (gp.blinky.x + (gp.blinky.hitbox.width/2)) - (gp.pacman.x + (gp.pacman.hitbox.width/2));
            int blinkyDistanceY = (gp.blinky.y + (gp.blinky.hitbox.height/2)) - (gp.pacman.y + (gp.pacman.hitbox.height/2));
            if (gp.pacman.direction == "up")
            {
                intermediateValueX = gp.pacman.hitbox.x + gp.pacman.hitbox.width/2 - 32;
                intermediateValueY = gp.pacman.hitbox.y + gp.pacman.hitbox.height/2 - 32;
            }
            else if (gp.pacman.direction == "left")
            {
                intermediateValueX = gp.pacman.hitbox.x + gp.pacman.hitbox.width/2 - 32;
                intermediateValueY = gp.pacman.hitbox.y + gp.pacman.hitbox.height/2;
            }
            else if (gp.pacman.direction == "down")
            {
                intermediateValueX = gp.pacman.hitbox.x + gp.pacman.hitbox.width/2;
                intermediateValueY = gp.pacman.hitbox.y + gp.pacman.hitbox.height/2 + 32;
            }
            else /* if pacman direction is right */
            {
                intermediateValueX = gp.pacman.hitbox.x + gp.pacman.hitbox.width/2 + 32;
                intermediateValueY = gp.pacman.hitbox.y + gp.pacman.hitbox.height/2;
            }

            /* final chase target values */
            this.targetX = intermediateValueX - blinkyDistanceX;
            this.targetY = intermediateValueY - blinkyDistanceY;
        }
        else if (this.ghostState == "scatter")
        {
            /* target is set to bottom right corner */
            this.targetX = 436;
            this.targetY = 524;
        }
        else if (this.ghostState == "eaten" || this.ghostState == "idleExit")
        {
            /* target is set to area right above ghost house */
            this.targetX = 224;
            this.targetY = 256;
        }

        /* no target for frightened or idle mode */
    }

    @Override
    public void update()
    {
        /* resets collision values */
        this.collisionOnRight = false;
        this.collisionOnLeft = false;
        this.collisionOnUp = false;
        this.collisionOnDown = false;

        //PATHFINDING, WALL COLLISION CHECKING, AND MOVEMENT
        this.calculateTarget();
        this.changeDirectionGhost(gp.cHandler);
        if (this.wallImmunity != true)
        {
            gp.cHandler.checkWallCollision(this);
        }
        this.moveGhost();

        //TELEPORT CHECKING
        this.teleport();

        //IDLE EXIT CHECKING
        if (levelTimer >= this.idleTime && this.ghostState == "idle")
        {
            this.changeGhostState("idleExit");
        }

        /* adjusts pinky's sprite timer (spriteCounter) for animations */
        spriteCounter++;
        if (spriteCounter > 6)
        {
            if (spriteNum < 4)
            {
                spriteNum++;
            }
            else /* if spriteNum == 2 */
            {
                spriteNum = 1;
            }
            spriteCounter = 0;
        }
    }

    /* graphics for Inky in game loop */
    @Override
    public void draw(Graphics2D g2)
    {
        image = null;

        /* eaten sprite animation */
        if (this.ghostState == "eaten")
        {
            switch (direction)
            {
                case "up":
                    image = eyesUp;
                    break;
                case "down":
                    image = eyesDown;
                    break;
                case "left":
                    image = eyesLeft;
                    break;
                case "right":
                    image = eyesRight;
                    break;
                case "stationary":
                    if (lastDirection == "up")
                    {
                        image = eyesUp;
                    } else if (lastDirection == "down")
                    {
                        image = eyesDown;
                    } else if (lastDirection == "left")
                    {
                        image = eyesLeft;
                    } else if (lastDirection == "right")
                    {
                        image = eyesRight;
                    }
                    break;
            }
        }
        else if (this.ghostState == "frightened" || (frightenedTimer >= 1 && this.frightenedTag == 0)) /* blue ghost animation */
        {
            /* blinking starts when 3 seconds left */
            if (frightenedTimer >= 360)
            {
                switch(spriteNum)
                {
                    case 1:
                        image = frightened1;
                        break;
                    case 2:
                        image = frightened2;
                        break;
                    case 3:
                        image = frightenedBlink1;
                        break;
                    case 4:
                        image = frightenedBlink2;
                        break;
                }
            }
            else
            {
                if (spriteNum % 2 != 0)
                {
                    image = frightened1;
                }
                else
                {
                    image = frightened2;
                }
            }
        }
        else /* normal ghost animation */
        {
            switch(direction)
            {
                case "up":
                    if (spriteNum % 2 != 0) /* odd number */
                        image = up1;
                    else
                        image = up2;
                    break;
                case "down":
                    if (spriteNum % 2 != 0)
                        image = down1;
                    else
                        image = down2;
                    break;
                case "left":
                    if (spriteNum % 2 != 0)
                        image = left1;
                    else
                        image = left2;
                    break;
                case "right":
                    if (spriteNum % 2 != 0)
                        image = right1;
                    else
                        image = right2;
                    break;
                case "stationary":
                    if (lastDirection == "up")
                    {
                        image = up2;
                    }
                    else if (lastDirection == "down")
                    {
                        image = down2;
                    }
                    else if (lastDirection == "left")
                    {
                        image = left2;
                    }
                    else if (lastDirection == "right")
                    {
                        image = right2;
                    }
                    break;
            }
        }

        g2.drawImage(image, x, y, null);

        /* hitbox visualizer - delete eventually */
//        g2.draw3DRect(this.hitbox.x, this.hitbox.y, this.hitbox.width, this.hitbox.height, true);
//        g2.draw3DRect(this.killHitbox.x, this.killHitbox.y, this.killHitbox.width, this.killHitbox.height, true);

        /* directional hitbox visualizer - delete ventually */
//        g2.draw3DRect(this.hitbox.x, this.hitbox.y - 18, hitboxSize + 2, hitboxSize + 2, true);
//        g2.draw3DRect(this.hitbox.x, this.hitbox.y + 18, hitboxSize + 2, hitboxSize + 2, true);
//        g2.draw3DRect(this.hitbox.x - 18, this.hitbox.y, hitboxSize + 2, hitboxSize + 2, true);
//        g2.draw3DRect(this.hitbox.x + 18, this.hitbox.y, hitboxSize + 2, hitboxSize + 2, true);

        /* target visualizer - delete eventually */
//        g2.draw3DRect(this.targetX, this.targetY, this.killHitbox.width, this.killHitbox.height, true);
    }
}
